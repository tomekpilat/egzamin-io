...
import { ArrowRight, BookOpenCheck, Database, GraduationCap, Info, RotateCcw, ShieldCheck, Target, Trophy } from "lucide-react";
...
export default function RecruitmentCalculatorPage() {
  ...
  return (
    <main className="calculator-page">
      ...
      <section className="calculator-hero"><div><h1>Kalkulator punktów do liceum</h1><p>Bez zakładania konta. Maksymalnie 200 punktów: 100 z egzaminu i 100 ze świadectwa oraz osiągnięć. Działa również dla technikum.</p></div></section>
      ...
      <nav id="calculator-flow" className="calculator-steps" aria-label="Etapy kalkulatora">
        <a href="#kalkulator" aria-current={activeStep === "points" ? "step" : undefined} onClick={(event) => { event.preventDefault(); goToStep("points"); }}><span>1.</span><p><b>{activeStep === "points" ? "Oblicz punkty" : `Punkty · ${formatPoints(result.total)}`}</b><small>Uzupełnij egzamin i świadectwo.</small></p></a>
        <a href="#szkola" aria-current={activeStep === "school" ? "step" : undefined} onClick={(event) => { event.preventDefault(); goToStep("school"); }}><span>2.</span><p><b>{activeStep === "result" && targetName ? targetName : "Znajdź szkołę"}</b><small>Wyszukaj lub zgłoś klasę.</small></p></a>
        <a href="#wynik" aria-current={activeStep === "result" ? "step" : undefined} onClick={(event) => { event.preventDefault(); goToStep("result"); }}><span>3.</span><p><b>Porównaj wynik</b><small>Zobacz różnicę względem progu.</small></p></a>
      </nav>
      ...
      <div className="calculator-shell" data-step={activeStep}>
        {activeStep === "points" ? <section id="kalkulator" className="calculator-step-panel calculator-form-column" aria-labelledby="calculator-points-title">
          ...
          <div className="calculator-points-layout"><Card className="calculator-input-card"><CardContent>
            ...
            <div className="calculator-certificate-card"><section className="calculator-input-section"><div className="calculator-section-heading"><GraduationCap aria-hidden="true" /><div><h2>Oceny na świadectwie</h2><p>Polski, matematyka i dwa przedmioty dla wybranej klasy.</p></div><b>{result.certificate.grades} / 72</b></div><div className="calculator-grid calculator-grid-two"><GradeField id="grade-polish" label="Język polski" value={form.grades[0]} onChange={(grade) => setGrade(0, grade)} /><GradeField id="grade-mathematics" label="Matematyka" value={form.grades[1]} onChange={(grade) => setGrade(1, grade)} /><GradeField id="grade-profile-one" label="Przedmiot profilowy 1" value={form.grades[2]} onChange={(grade) => setGrade(2, grade)} /><GradeField id="grade-profile-two" label="Przedmiot profilowy 2" value={form.grades[3]} onChange={(grade) => setGrade(3, grade)} /></div></section>

            <section className="calculator-input-section"><div className="calculator-section-heading"><Trophy aria-hidden="true" /><div><h2>Dodatkowe punkty</h2><p>Wyróżnienie, wolontariat i osiągnięcia.</p></div><b>{formatPoints(result.certificate.honors + result.certificate.volunteering + result.certificate.achievements)} / 28</b></div><div className="calculator-extras"><label className="calculator-check-row" htmlFor="honors"><Checkbox id="honors" checked={form.honors} onCheckedChange={(checked) => setForm((c) => ({ ...c, honors: checked === true }))} /><span>Świadectwo z wyróżnieniem <small>+7 punktów</small></span></label><label className="calculator-check-row" htmlFor="volunteering"><Checkbox id="volunteering" checked={form.volunteering} onCheckedChange={(checked) => setForm((c) => ({ ...c, volunteering: checked === true }))} /><span>Aktywność społeczna lub wolontariat <small>+3 punkty</small></span></label><div className="calculator-achievements"><div><Label htmlFor="achievements">Szczególne osiągnięcia i konkursy</Label><small>Maksymalnie 18 punktów.</small></div><Input id="achievements" type="number" inputMode="decimal" min={0} max={18} value={form.achievements} onChange={(event) => setForm((c) => ({ ...c, achievements: Number(event.target.value) }))} /></div></div></section></div>
          </CardContent></Card>
          ...
          <div className="calculator-step-actions"><Button type="button" variant="ghost" className="calculator-reset" onClick={reset}><RotateCcw aria-hidden="true" /> Wyczyść</Button></div>
        </section> : null}
        ...
        {activeStep !== "points" ? <section id={activeStep === "school" ? "szkola" : "wynik"} className="calculator-step-panel calculator-result-column" aria-labelledby={activeStep === "school" ? "calculator-school-title" : "calculator-result-title"}>
          ...
          <Card className="calculator-result-card">{activeStep === "result" ? <CardHeader className="calculator-comparison-summary"><div><span>Twoje punkty</span><strong>{formatPoints(result.total)}</strong></div><div><span>{comparison ? `Próg${verifiedThreshold ? ` ${verifiedThreshold.recruitment_year}` : ""}${targetName ? ` · ${targetName}` : ""}` : "Wybrany próg"}</span><strong>{comparison ? formatPoints(comparison.threshold) : "—"}</strong></div><div><span>Różnica</span><strong className={signedDifference === null ? "" : signedDifference >= 0 ? "positive" : "negative"}>{signedDifference === null ? "—" : `${signedDifference > 0 ? "+" : ""}${formatPoints(signedDifference)}`}</strong></div></CardHeader> : null}<CardContent>
          {activeStep === "result" && comparison ? <div className="calculator-comparison-scale" aria-label={`Twoje punkty ${formatPoints(result.total)}, próg ${formatPoints(comparison.threshold)}`}><div><span style={{ width: `${Math.min(100, result.total / 2)}%` }} /><i style={{ left: `${Math.min(100, comparison.threshold / 2)}%` }} /></div><p><span>0</span><span>próg {formatPoints(comparison.threshold)}</span><span>200</span></p></div> : null}
          ...
          {activeStep === "result" ? <div className="calculator-plan-callout"><p><b>Egzamin to aż 100 z 200 punktów.</b> Ćwicz zadania CKE i pytaj AI dokładnie tam, gdzie utkniesz.</p><div className="calculator-role-actions"><Button size="lg" asChild><a href="/logowanie?tryb=rejestracja&rola=uczen">Ćwicz z arkuszami CKE <ArrowRight aria-hidden="true" /></a></Button><Button size="lg" variant="outline" asChild><a href="/logowanie?tryb=rejestracja&rola=rodzic">Jestem rodzicem — jak pomóc?</a></Button></div></div> : null}
