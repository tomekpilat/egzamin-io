...
export function AnalyticsConsent({ measurementId }: { measurementId?: string }) {
  ...
  return <>
    {choice === "pending" ? <section className="analytics-consent-banner" role="dialog" aria-modal="false" aria-labelledby="analytics-consent-title"><div><b id="analytics-consent-title">Czy możemy mierzyć użycie strony?</b><p>Google Analytics pomoże nam ulepszać egzaminio. Nie wysyłamy ocen, odpowiedzi, rozmów AI, danych konta ani pełnych adresów URL. Bez zgody Google Analytics nie jest ładowane.</p><a href="/polityka-cookies">Polityka cookies</a></div><div className="analytics-consent-actions"><Button variant="outline" onClick={() => save("accepted")}>Akceptuję</Button><Button variant="outline" onClick={() => save("rejected")}>Odrzucam opcjonalne</Button><Button variant="outline" onClick={() => setSettingsOpen(true)}>Ustawienia</Button></div></section> : null}
