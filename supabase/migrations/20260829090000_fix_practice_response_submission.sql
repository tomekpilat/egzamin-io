-- Final, self-contained practice submission RPC for standard-paper MVP.
-- This removes runtime dependence on the retired accommodation preference and
-- on a chain of renamed wrapper functions left by earlier migrations.
...
begin
  ...
  -- This row lock serializes quota checks and submissions for one student.
  perform 1
  ...
  return query select
    ...
    (select count(*)::integer from public.student_question_attempts attempt where attempt.student_id = current_student_id and attempt.is_correct),
    awarded,
    maximum,
    status_value;
end;
